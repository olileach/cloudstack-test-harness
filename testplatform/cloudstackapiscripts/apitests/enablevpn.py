...
if __name__ == "__main__":
    ...
    enablevpn = cs_api.request(dict({'command':'createRemoteAccessVpn', 'publicipid' : 10 }))
    ...
    publicip, publicipid, iprange, presharedkey, account, state
    = None, None, None, None, None, None

    for key, value in enablevpn_asyncqry.iteritems():
        print key, value
        
        if key == 'publicip' : ipid = value
        if key == 'publicipid' : pubstartport = value
        if key == 'iprange' : pubendport = value
        if key == 'presharedkey' : privstartport = value
        if key == 'account' : privendport = value
